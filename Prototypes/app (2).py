...
def create_app():
    ...
    def _auth_and_language():
        ...
        if request.path.startswith('/static/') or request.endpoint in ('login', 'logout', 'set_lang', 'showroom', 'showroom_product'):
            pass
        else:
...
def register_routes(app):
    ...
    def showroom():
        ...
        Shows only in-stock, non-deleted products. Optional ?category=... filter.
        Social links are configured via SHOP_INSTAGRAM_URL / SHOP_FACEBOOK_URL
        env vars; a button is only shown for whichever is set.
        """
        category = request.args.get("category", "").strip()
        in_stock = [p for p in active_products() if p.stock > 0]
        categories = sorted({p.category for p in in_stock if p.category})
        items = in_stock
        if category:
    ...
    def showroom_product(pid):
        """Public product detail page. Same visibility rules as the listing:
        soft-deleted or out-of-stock products are treated as not found.
        """
        product = Product.query.get_or_404(pid)
        if product.is_deleted or product.stock <= 0:
            abort(404)
        related = [
            p for p in active_products()
            if p.id != product.id and p.stock > 0 and p.category == product.category
        ][:4]
        return render_template(
            "showroom_product.html",
            product=product,
            related=related,
            instagram_url=os.getenv("SHOP_INSTAGRAM_URL", "").strip(),
            facebook_url=os.getenv("SHOP_FACEBOOK_URL", "").strip(),
